...
describe('providersApi', () => {
  ...
  it('listProviders calls GET /api/v1/providers without projectId', async () => {
    vi.spyOn(client, 'get').mockResolvedValue([{ id: '1', name: 'Test' }] as any)
  ...
  it('addProvider calls POST /api/v1/providers', async () => {
    vi.spyOn(client, 'post').mockResolvedValue({ id: '1', name: 'New' } as any)
  ...
  it('updateProvider calls PATCH /api/v1/providers/:id', async () => {
    vi.spyOn(client, 'patch').mockResolvedValue({ id: '1', name: 'Updated' } as any)
  ...
  it('testProvider calls POST /api/v1/providers/:id/test', async () => {
    vi.spyOn(client, 'post').mockResolvedValue({ success: true, valid: true, message: 'OK' } as any)
  ...
  it('setDirector calls PATCH /api/v1/providers/:id/directorship', async () => {
    vi.spyOn(client, 'patch').mockResolvedValue({ id: '1', is_project_director: true } as any)
  ...
  it('getProviderAgents calls GET /api/v1/providers/:id/agents', async () => {
    vi.spyOn(client, 'get').mockResolvedValue([{ id: '1', name: 'Agent' }] as any)
  ...
  it('resolveProvider calls POST /api/v1/providers/resolve', async () => {
    vi.spyOn(client, 'post').mockResolvedValue({ provider: 'claude', model: 'gpt-4o' } as any)
