...
describe('ErrorPage', () => {
  ...
  it('renders 404 title by default', () => {
    vi.mocked(useRoute).mockReturnValue({ query: { error: '404' } } as unknown as ReturnType<typeof useRoute>)
    const wrapper = mount(ErrorPage)
    expect(wrapper.text()).toContain('Page Not Found')
  })

  it('renders 500 title when ?error=500', () => {
    vi.mocked(useRoute).mockReturnValue({ query: { error: '500' } } as unknown as ReturnType<typeof useRoute>)
    const wrapper = mount(ErrorPage)
