...
test_approvals_api() {
  ...
  curl_sf -X POST "${BASE}/api/v1/projects/$proj_id/tickets/$ticket_id/status" \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer $admin_token" \
  ...
  local approval_body
  approval_body=$(curl_sf -X POST "${BASE}/api/v1/approvals" \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer $admin_token" \
  ...
  local pending_body
  pending_body=$(curl_sf "${BASE}/api/v1/approvals/pending" \
    -H "Authorization: Bearer $admin_token")
  assert_has_field "Get pending approvals" "data" "$pending_body"

  local approve_body
  approve_body=$(curl_sf -X POST "${BASE}/api/v1/approvals/$approval_id/approve" \
    -H "Authorization: Bearer $admin_token")
