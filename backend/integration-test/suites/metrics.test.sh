...
test_metrics() {
  ...
  # Make a request first to populate histogram (slowRequestLogger only covers /api/v1/*)
  curl_sf "$BASE/api/health" >/dev/null 2>&1

  # 1. GET /metrics returns text/plain content-type
  local metrics_headers
  metrics_headers=$(curl -s -D - "$BASE/metrics" 2>/dev/null | sed -e '/^$/q')
  if [[ "$metrics_headers" == *[Cc]ontent-[Tt]ype:*text/plain* ]] || echo "$metrics_headers" | grep -qi "content-type:.*text/plain"; then
    pass "/metrics returns text/plain content-type"
  else
    fail "/metrics content-type" "expected text/plain, got: $(echo "$metrics_headers" | grep -i content-type || echo 'none')"
  fi

  # 2-6. Check metrics body (extract body after HTTP headers)
  local metrics_body
  metrics_body=$(curl -s "$BASE/metrics" 2>/dev/null)
  echo "$metrics_body" > /tmp/metrics_debug.txt 2>/dev/null

  # 2. Response contains http_request_duration_seconds
  if [[ "$metrics_body" == *"http_request_duration_seconds"* ]]; then
    pass "/metrics contains http_request_duration_seconds histogram"
  else
    fail "/metrics histogram" "missing http_request_duration_seconds (body length: ${#metrics_body})"
  fi

  # 3. Response contains http_requests_total counter
  if [[ "$metrics_body" == *"http_requests_total"* ]]; then
  ...
  if [[ "$metrics_body" == *"db_pool_total"* ]]; then
  ...
  if [[ "$metrics_body" == *"db_pool_idle"* ]]; then
  ...
  if [[ "$metrics_body" == *"db_pool_waiting"* ]]; then
