...
if [ -z "$AGENT_ID" ] || [ -z "$AGENT_KEY" ]; then
  fail "Could not create agent" "$CREATE_RESPONSE"
  exit 1
fi
...
if echo "$KEY_RESPONSE" | grep -q '"success":true\|"agents"\|"error"'; then
  if echo "$KEY_RESPONSE" | grep -q '"error"'; then
    fail "Agent key not working" "$KEY_RESPONSE"
    exit 1
  fi
  pass "Agent key is valid"
else
  fail "Agent key not working" "$KEY_RESPONSE"
  exit 1
fi
...
if [ -z "$NEW_KEY" ]; then
  fail "Key rotation failed" "$ROTATE_RESPONSE"
  exit 1
fi

pass "New Key: ${NEW_KEY:0:10}..."
...
if echo "$OLD_KEY_RESPONSE" | grep -q '"error"'; then
  pass "Old key rejected"
else
  fail "Old key still works" "$OLD_KEY_RESPONSE"
  exit 1
fi
...
if echo "$NEW_KEY_RESPONSE" | grep -q '"success":true\|"agents"'; then
  if echo "$NEW_KEY_RESPONSE" | grep -q '"error"'; then
    fail "New key not working" "$NEW_KEY_RESPONSE"
    exit 1
  fi
  pass "New key is valid"
else
  fail "New key not working" "$NEW_KEY_RESPONSE"
  exit 1
fi
...
echo ""
pass "API key rotation integration test complete"
