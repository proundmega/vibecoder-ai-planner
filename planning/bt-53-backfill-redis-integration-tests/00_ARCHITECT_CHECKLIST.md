...
- [ ] **Related ticket**: bp-53 (Distributed State) is completed before starting this backfill
...
- [ ] I have checked if there are **existing tabs/sections** where this feature could be added
- [ ] I have checked if the **feature should extend existing code** rather than creating new files
- [ ] I have checked if **shared components** exist that I should reuse
- [ ] I have checked existing test files: `backend/src/__tests__/`, `backend/src/middleware/*.test.js`
...
- [ ] I have checked the **OpenAPI spec** and know if JSDoc annotations are needed
- [ ] I have checked if **generated TypeScript types** need regeneration
- [ ] I have checked if **frontend API response validation** needs updating
...
- [ ] **Bash integration suite**: test added or extended in `backend/integration-test/suites/`
...
- [ ] Generated files regenerated if applicable
...
3. **Conflicting requirements** — this best practice conflicts with an existing feature
4. **Unknown unknowns** — you discover something during implementation that fundamentally changes the approach
5. **Production impact** — the change could affect running users
6. **UI placement decision** — you need user input on UI placement
7. **Backend API conflict** — the backend API doesn't exist and creating it would conflict
8. **Model can't resolve** — the local model fails repeatedly
